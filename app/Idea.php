...
class Idea extends Model
{
    ...
    {
        return $this->designTasks()->orderBy('created_at', 'desc')->take(3);
    }
    ...
    public function getLatestPhaseAttribute()
    {
        if ($this->plan_state() == 'open') {
            return 'Plan Phase';
        } elseif ($this->design_state() == 'open') {
            return 'Design Phase';
        } elseif ($this->support_state() == 'open') {
            return 'Support Phase';
        } elseif ($this->support_state() == 'failed') {
            return 'Failed';
        }
    ...
    public function addDesignTask($name, $description, $xmovement_task_type)
    {
        switch ($xmovement_task_type) {
            case 'Poll':
                if (!DynamicConfig::fetchConfig('XMOVEMENT_POLL')) {
                    return;
                }
        $proposal_interactivity = true;
            ...
            case 'Discussion':
                if (!DynamicConfig::fetchConfig('XMOVEMENT_DISCUSSION')) {
                    return;
                }
    ...
    public function support_state()
    {
        if ($this->freeze_state) {
    ...
    public function tender_state()
    {
        if ($this->freeze_state) {
            return $this->tender_state;
        }
